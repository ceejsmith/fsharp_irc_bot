...
let parse (reply: string) =
    if reply.StartsWith("PING") then
        Ping(reply.Substring(5))
    else if reply.StartsWith("ERROR") then
        Error(reply.Substring(6))
    else if reply.StartsWith(":") then
        Message(reply.Substring(1))
    else
        Unknown
